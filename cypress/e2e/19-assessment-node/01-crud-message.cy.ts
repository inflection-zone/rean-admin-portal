...
describe('test', () => {
    ...
    cy.wait(3000)
      ...
      cy.wait(2000)
      cy.get('input[name=title]').type('Visit to doctor recommended')
      cy.wait(2000)
      cy.get('textarea[name=description]').type('Description of template')
      cy.wait(2000)
      cy.get('input[name=sequence]').type('5')
      cy.wait(2000)
      cy.get('textarea[name=message]').type('Please see your doctor this weekend')
      cy.wait(2000)
      cy.get('button[type=submit]').click()
      cy.wait(2000)
      cy.get('button[type=submit]').click()
