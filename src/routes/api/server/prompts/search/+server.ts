...
import { searchPrompts } from '../../../services/prompts';
...
export const GET = async (event: RequestEvent) => {
	...
	try {
		...
		const response = await searchPrompts(sessionId, searchParams);
		const items = response.Data.Prompts.Items;
		console.log('res ==', response);

		return new Response(JSON.stringify(items));
	} catch (err) {
		console.error(`Error retriving Prompts: ${err.message}`);
