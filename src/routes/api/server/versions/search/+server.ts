...
import { searchVersions } from '../../../services/versions';
...
export const GET = async (event: RequestEvent) => {
    ...
    try {
        ...
        const response = await searchVersions(sessionId, searchParams);
        const items = response.Data.Versions.Items;
        console.log('res ==', response);

        return new Response(JSON.stringify(items));
    } catch (err) {
        console.error(`Error retriving Versions: ${err.message}`);
