...
import { searchSystems } from '../../../services/systems';
...
export const GET = async (event: RequestEvent) => {
	...
	try {
		...
		const response = await searchSystems(sessionId, searchParams);
		const items = response.Data.Systems.Items;
		console.log('res ==', response);

		return new Response(JSON.stringify(items));
	} catch (err) {
		console.error(`Error retriving Systems: ${err.message}`);
