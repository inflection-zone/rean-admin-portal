<script lang="ts">
    ...
    let itemsPerPage = 10;
    let offset = 0;
    let totalDocumentsCount = data.documents.TotalCount;
    ...
    async function searchDocuments(model) {
        ...
        documents = response.map((item, index) => ({ ...item, index: index + 1 }));
    }

    $: {
        documents = documents.map((item, index) => ({ ...item, index: index + 1 }));
        paginationSettings.size = data.documents.TotalCount;
        retrivedDocuments = documents.slice(
    ...
    async function Delete(model) {
        const response = await fetch(`/api/server/documents`, {
            method: 'DELETE',
