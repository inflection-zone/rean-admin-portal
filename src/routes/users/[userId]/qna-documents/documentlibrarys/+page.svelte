<script lang="ts">
    ...
    let retrivedSystems;
    $:systems = data.systems.Items;

    const userId = $page.params.userId;
    const qnadocumentsRoute = `/users/${userId}/qna-documents/systems`; 
    const editRoute = (id) => `/users/${userId}/qna-documents/systems/${id}/edit`;
    const viewRoute = (id) => `/users/${userId}/qna-documents/systems/${id}/view`;
    const createRoute = `/users/${userId}/qna-documents/systems/create`;
    ...
    let itemsPerPage = 10;
    let offset = 0;
    let totalSystemsCount = data.systems.TotalCount;
    ...
    let paginationSettings = {
        page: 0,
        limit: 10,
        size: totalSystemsCount,
        amounts: [10, 20, 30, 50]
    } satisfies PaginationSettings;

    async function searchSystem(model) {
        let url = `/api/server/systems/search?`;
        ...
        systems = response.map((item, index) => ({ ...item, index: index + 1 }));
    }

    $: {
        systems = systems.map((item, index) => ({ ...item, index: index + 1 }));
        paginationSettings.size = data.systems.TotalCount;
        retrivedSystems = systems.slice(
    ...
    async function Delete(model) {
        const response = await fetch(`/api/server/systems`, {
            method: 'DELETE',
...
<div class="table-container my-2 !border !border-secondary-100 dark:!border-surface-700">
    ...
    >
        ...
        <tbody class="!bg-white dark:!bg-inherit">
            {#if retrivedSystems.length <= 0}
                <tr>
                    <td colspan="6">No records found</td>
                </tr>
            {:else}
                {#each retrivedSystems as row}
