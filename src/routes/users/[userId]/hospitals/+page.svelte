<script lang="ts">
	...
	async function searchHospital(model) {
		...
		if (hospitalName) url += `&hospitalName=${hospitalName}`;
		...
		hospitals = response.map((item, index) => ({ ...item, index: index + 1 }));
	}
					...
					<tr class="!border-b !border-b-secondary-100 dark:!border-b-surface-700">
						...
						<td role="gridcell" aria-colindex={2} tabindex="0">
							<a href={viewRoute(row.id)}>
								{row.HospitalName !== null && row.HospitalName !== ''
									? Helper.truncateText(row.HospitalName, 20)
									: 'Not specified'}
							</a>
						</td>
